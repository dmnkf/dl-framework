# Copied from: https://github.com/oetu/MMCL-ECG-CMR/blob/main/mmcl/utils/ecg_augmentations.py
        ...
        if self.resize == True:
            ...
            if sample_dims == 2:
                for ch in range(sample.shape[-2]):
                    resized_signal = np.interp(
                        np.linspace(0, crop_len, num=resize_len),
                        np.arange(crop_len),
                        sample[ch, start_idx : start_idx + crop_len],
                    )
                ...
                for f_bin in range(sample.shape[-3]):
                    for ch in range(sample.shape[-2]):
                        resized_signal = np.interp(
                            np.linspace(0, crop_len, num=resize_len),
                            np.arange(crop_len),
                            sample[f_bin, ch, start_idx : start_idx + crop_len],
                        )
